...
export class AlertComponent implements OnInit, OnDestroy {
    ...
    // this enables the alert component to be notified whenever an alert message is sent to the alert service and add it to the alerts array for display
    // Alerts are cleared when an alert with an empty message is received from the alert service.
    // also calls router.events.subscribe() to subscribe to route change events so it can automatically clear alerts on route changes.
    ngOnInit() {
